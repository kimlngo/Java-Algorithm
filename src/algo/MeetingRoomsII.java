...
//        System.out.println(minMeetingRooms(new int[][]{{13, 15}, {1, 13}}));
    ...
    private static int minMeetingRooms(int[][] intervals) {
        //special case, only 1 interval
        if (intervals.length == 1) return 1;

        //1) convert intervals into List<Meeting>
        ...
        //3) run through the iteration Points,
        //if the # of room count > max ==> store as max
        for (int t : iterationPoints) {
            final int finalT = t;
            int count = (int) meetings.stream()
                                      .filter(meeting -> meeting.from() <= finalT && finalT < meeting.to())
                                      .count();
            if (count > maxCount) {
                maxCount = count;
            }
    ...
    private static List<Meeting> convertAndSortMeeting(int[][] intervals) {
        return Arrays.stream(intervals)
                     .map(itv -> new Meeting(itv[0], itv[1]))
                     .sorted(Comparator.comparingInt(Meeting::from)
                                       .thenComparingInt(Meeting::to))
                     .toList();
    }
}

record Meeting(int from, int to) {
}
